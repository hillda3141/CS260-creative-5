document.getElementById("addressSubmit").addEventListener("click", function(event) {
    ...
    let value = document.getElementById("addressInput").value.toLowerCase();
    if (value === "")
      return;
        ...
        return response.json();
          ...
          if(result.length === 0){
            ...
            for(let i = 0; i < result.length; i++){
              let c = "";
              let type = "Undefined";
              if(result[i].from.toLowerCase() == value){
                c = "loss";
                type = "Transfer Out"
              }
              else if(result[i].to.toLowerCase() == value){
                c = "gain";
                type = "Transfer In"
              }
              html_text += "<div class='transaction-container " + c + "'>";
              html_text += "<h2>" + type + "</h2>"
              ...
              if(c === "loss"){
                ...
                html_text += "<h6>To: " + result[i].to +"</h6>";
              }
              else if(c == "gain"){
                html_text += "<h4 class=" + c + ">Amount: " + amount +"</h4>";
                html_text += "<h6>From: " + result[i].from +"</h6>";
              }
              html_text += "</div>"
            }
          }
